...
Definition subst_abs (L : lambda_theory) {m n : nat} (f : L (S m)) (g : stn m → L n)
  : (abs f) • g = abs (f • ((λ i, inflate (g i)) ::f (last var)))
  := !abs_subst _ _ _.

Definition inflate_abs (L : lambda_theory) {n : nat} (f : L (S n))
  : inflate (abs f) = abs (f • ((init (init var)) ::f (last var))).
Proof.
  unfold inflate.
  rewrite subst_abs.
  apply (maponpaths (λ x, abs (f • (x ::f _)))).
  apply funextfun.
  intro i.
  apply inflate_var.
Qed.
...
Section AppAbs.
  ...
  Lemma app_from_app'
    {n : nat}
    (s : (L n : hSet))
    : appx s = extended_composition (app' L) [s].
  Proof.
  ...
  Local Lemma aux1
    ...
    {s : (L (S n) : hSet)}
    {t : (L n : hSet)}
    (H2 : extended_composition (app' L) [t] = s)
    : abs s = app' L • [lift_constant n (one L) ; t].
  Proof.
  ...
  Lemma abs_from_one
    ...
    (t : (L n : hSet))
    : abs s = t
    ≃ (app' L • [lift_constant _ (one L) ; t] = t)
      × (extended_composition (app' L) [t] = s).
  Proof.
    use (logeqweq
      (make_hProp _ (setproperty _ _ _))
      (make_hProp _ (isapropdirprod _ _ (setproperty _ _ _) (setproperty _ _ _)))).
    - intro H1.
      assert (H2 : extended_composition (app' L) [t] = s).
      {
