(**
  ...
  This file constructs the category of rings, and gives a constructor for ring isomorphisms.

  Contents
  1. The category of rings
  1.1. The category of rings [ring_category]
  1.2. Univalence [ring_category_is_univalent]
  2. A constructor for ring isomorphisms [make_ring_z_iso]
...
(** ** 2. A constructor for ring isomorphisms *)

Lemma isringfun_z_iso_inv
  {X Y : ring}
  (f : z_iso (C := HSET) (X : hSet) (Y : hSet))
  (H : isringfun (z_iso_mor f))
  : isringfun (inv_from_z_iso f).
Proof.
  apply make_isrigfun.
  - apply (ismonoidfun_z_iso_inv (X := ringaddabgr X) (Y := ringaddabgr Y) f).
    exact (isrigfunisaddmonoidfun H).
  - apply (ismonoidfun_z_iso_inv (X := ringmultmonoid X) (Y := ringmultmonoid Y) f).
    exact (isrigfunismultmonoidfun H).
Qed.

Definition make_ring_z_iso
  {X Y : ring}
  (f : z_iso (C := HSET) (X : hSet) (Y : hSet))
  (H : isringfun (z_iso_mor f))
  : z_iso (C := ring_category) X Y.
Proof.
  use make_z_iso.
  - exact (rigfunconstr H).
  - exact (rigfunconstr (isringfun_z_iso_inv f H)).
  - abstract (
      split;
      apply rigfun_paths;
